...
def main(args):
    ...
    drs = DataRequest.objects.filter(
        climate_model__short_name__in=['MetUM-GOML2-LR', 'MetUM-GOML2-HR'],
        experiment__short_name__in=['primWP5-amv-pos', 'primWP5-amv-neg'],
        variable_request__table_name__in=['Amon', 'Omon'],
        variable_request__cmor_name__in=['hfls', 'hfss', 'pr', 'psl', 'rlds',
                                         'rlus', 'rlut', 'rsds', 'rsdt',
                                         'rsus', 'rsut', 'tas', 'hus', 'ta',
                                         'ua', 'va', 'wap', 'zg', 'hfcorr',
                                         'mlotst', 'wfcorr']
    )
    rr.data_request.add(*drs)
