...
"""
update_dreqs_0070.py

This script creates new source_ids for MPI-M and updates existing coupled
...
def main(args):
    ...
    Main entry point
    """
    hr = ClimateModel.objects.create(short_name='MPI-ESM1-2-HR',
                                     full_name='MPI-ESM1-2-HR')
    xr = ClimateModel.objects.create(short_name='MPI-ESM1-2-XR',
                                     full_name='MPI-ESM1-2-XR')
    ...
    num_updated = DataRequest.objects.filter(
        ...
        experiment__short_name__in=['control-1950', 'highres-future',
                                    'hist-1950', 'spinup-1950',
                                    'highresSST-future']
    ).update(climate_model=xr)

    logger.debug('{} DataRequests updated to use {}'.format(num_updated, xr))
