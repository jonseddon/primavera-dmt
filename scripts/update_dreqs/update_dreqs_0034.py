...
"""
update_dreqs_0034.py

This file adds data and variable requests for the Ofx table, which was missed
...
from cf_units import date2num, CALENDAR_PROLEPTIC_GREGORIAN, CALENDAR_360_DAY
...
def main(args):
    ...
    new_vreqs = [
        ...
        'uid': 'babcc39c-e5dd-11e5-8482-ac72891c3257'},
        {'table_name': 'Ofx',
        ...
        'uid': 'baa3f2e0-e5dd-11e5-8482-ac72891c3257'},
    ]
    ...
    mohc_details = {
        'id': 'MOHC',
        'model_ids': ['HadGEM3-GC31-HM', 'HadGEM3-GC31-MM',
                      'HadGEM3-GC31-LM'],
    ...
    # Institutes
    result = match_one(Institute, short_name=mpi_details['id'])
    if result:
        mpi = result
    else:
    ...
    result = match_one(Institute, short_name=mohc_details['id'])
    if result:
        mohc = result
    else:
        msg = 'institute_id {} not found in the database.'.format(
    ...
    mpi_model_objs = []
    for clim_model in mpi_details['model_ids']:
        result = match_one(ClimateModel, short_name=clim_model)
        if result:
            mpi_model_objs.append(result)
        else:
    ...
    mohc_model_objs = []
    for clim_model in mohc_details['model_ids']:
        result = match_one(ClimateModel, short_name=clim_model)
        if result:
            mohc_model_objs.append(result)
        else:
            msg = ('climate_model {} not found in the database.'.
                   format(clim_model))
            print msg
    ...
    for new_dreq in mpi_dreqs:
        ...
        if var_req_obj:
            for expt in experiment_objs:
                for clim_model in mpi_model_objs:
                    _dr = get_or_create(
                        DataRequest,
                        project=project,
                        institute=mpi,
                        ...
                        request_start_time=date2num(
                            ...
                            std_units, mpi_details['calendar']
                        ),
    ...
    for new_dreq in mohc_dreqs:
        ...
        if var_req_obj:
            for expt in experiment_objs:
                for clim_model in mohc_model_objs:
                    _dr = get_or_create(
                        DataRequest,
                        project=project,
                        institute=mohc,
                        ...
                        request_start_time=date2num(
                            experiments[expt.short_name]['start_date'],
                            std_units, mohc_details['calendar']
                        ),
                        request_end_time=date2num(
                            experiments[expt.short_name]['end_date'],
                            std_units, mohc_details['calendar']
                        ...
                        time_units=std_units,
                        calendar=mohc_details['calendar']
                    )
        else:
            msg = ('Unable to find variable request matching '
                   'cmor_name {} and table_name {} in the '
